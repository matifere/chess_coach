...
- 🧠 **Cerebro en Rust (Alpha-Beta + NNUE):** Motor de búsqueda personalizado de alto rendimiento que evalúa posiciones usando la red neuronal NNUE de forma local.
...
- 🎨 **Gráficos de Alta Calidad & Feedback Visual:** Gráficos vectoriales (SVG) de Lichess. Además, las casillas de origen/destino se iluminan y se muestran *badges* de calidad de movimiento (!!, ??, ?!).
- 🖐️ **Drag & Drop Integrado:** Juega con clics tradicionales o arrastrando las piezas; las animaciones responden inteligentemente para no interferir con las físicas de arrastre.
...
- **[Rust](https://www.rust-lang.org/):** Lenguaje utilizado para el motor de ajedrez (búsqueda Min-Max Alpha-Beta).
- **[flutter_rust_bridge](https://fzyzcjy.github.io/flutter_rust_bridge/):** Puente asíncrono y síncrono ultra-rápido entre Dart y Rust.
- **[nnue-rs](https://crates.io/crates/nnue-rs) & [shakmaty](https://crates.io/crates/shakmaty):** Librerías de Rust para manejo eficiente de tableros e inferencia local de redes neuronales de ajedrez.
- **[flutter_bloc](https://pub.dev/packages/flutter_bloc):** Manejo de estados de la aplicación utilizando la arquitectura *Cubit*.
- **[chess (Dart)](https://pub.dev/packages/chess):** Lógica interna base de validación y PGN.
...
2. Entra al directorio del proyecto:
   ...
   cd chess_coach
   ```
3. Genera los bindings de Rust (opcional si ya están generados):
   ```bash
...
- [ ] Vincular las clasificaciones visuales reales de las jugadas (Excelente, Error, Blunder) con el diferencial de evaluación NNUE de Rust.
- [ ] Evaluar múltiples líneas principales (MultiPV) para simular el comportamiento de un "entrenador".
